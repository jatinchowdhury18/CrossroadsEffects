...
from gen_faust import Model,Element,Gain,Delay,Split,Feedback,get_error_for_model
...
ys.append(y1)
names.append('gain')
wavfile.write(names[-1] + '.wav', fs, y1)
...
ys.append(y2)
names.append('delay')
wavfile.write(names[-1] + '.wav', fs, y2)
...
plugin = 'param_est'
orig_file = 'drums.wav'
out_file = 'param_est.wav'
# Estimate all structures
for n in range(len(models)):
    print('Estimating params for: {}'.format(names[n]))
    params = estimate_params(models[n], plugin, orig_file, out_file, names[n] + '.wav')
    models[n].set_params(params)

    err = get_error_for_model(params, models[n], plugin, orig_file, out_file, names[n] + '.wav')
